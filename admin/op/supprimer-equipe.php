...
if(!isset($_GET["all"])){
    ...
    if(!empty($_GET["id"])){
        $id = intval($_GET["id"]) ;
        $req = $bdd->prepare("DELETE FROM equipes WHERE idEquipe = ?") ;
        try{
        ...
        $result["error"] = "Veuillez choisir une équipe à supprimer" ;
    }
    ...
    try{
        ...
        $_SESSION["success"] = "Les equipes ont été supprimées avec succès" ;
    }catch(Exception $e){
